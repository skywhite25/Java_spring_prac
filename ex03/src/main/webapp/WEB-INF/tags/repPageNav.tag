...
<ul class="pagination">
  	<li data-page=1>
		<c:if test="${pageObject.page > 1 }">
	  		<a href="${listURI }?${query}&repPage=1&perRepPageNum=${pageObject.perPageNum}&mode=1"
	...
	<li data-page=${pageObject.startPage -1 }>
		<c:if test="${pageObject.startPage > 1 }">
	  		<a href="${listURI }?${query}&repPage=${pageObject.startPage - 1 }&perRepPageNum=${pageObject.perPageNum}&mode=1"
	...
	<c:forEach begin="${pageObject.startPage }" end="${pageObject.endPage }" var="cnt">
  	<li ${(pageObject.page == cnt)?"class=\"active\"":"" } 
  	 data-page=${cnt }>
  	 	...
  	 	<c:if test="${pageObject.page != cnt }">
  			<a href="${listURI }?${query}&repPage=${cnt }&perRepPageNum=${pageObject.perPageNum}&mode=1"
	...
	<c:if test="${pageObject.endPage < pageObject.totalPage }">
	  	<li data-page=${pageObject.endPage + 1 }>
	  		<a href="${listURI }?${query}&repPage=${pageObject.endPage + 1 }&perRepPageNum=${pageObject.perPageNum}&mode=1"
	...
	<c:if test="${pageObject.page < pageObject.totalPage }">
	  	<li data-page=${pageObject.totalPage }>
	  		<a href="${listURI }?${query}&repPage=${pageObject.totalPage }&perRepPageNum=${pageObject.perPageNum}&mode=1"
